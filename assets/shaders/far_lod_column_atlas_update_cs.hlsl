...
void resolveFarLodColumnBlocks(FarLodGpuBiome biome, int surfaceY, float distanceToShore, int seaLevel, int worldX, int worldZ, out uint surfaceBlock, out uint fillerBlock)
{
...
{
    TerrainPointSample result = (TerrainPointSample)0;
    ...
    const float distanceToShore = isOcean ? abs(signedDistanceToCoast) : max(signedDistanceToCoast, 0.0f);
    ...
    result.hasSolid = 1u;
    result.waterEnabled = (biome.flags & kFarLodBiomeWaterFill) != 0u ? 1u : 0u;
    result.waterBottomY = max(result.surfaceY + 1, header.seaLevel - biome.waterMaxDepth + 1);
    return result;
}

GpuTerrainAtlasSample buildAtlasSample(int worldX, int worldZ)
{
    GpuTerrainAtlasSample result = (GpuTerrainAtlasSample)0;
    const TerrainPointSample sample = sampleTerrainPoint(worldX, worldZ);
    result.hasSolid = sample.hasSolid;
    result.waterEnabled = sample.waterEnabled;
    result.surfaceY = sample.surfaceY;
    result.waterBottomY = sample.waterBottomY;
    result.surfaceBlock = sample.surfaceBlock;
    result.fillerBlock = sample.fillerBlock;
    return result;
}
...
void FarLodColumnAtlasUpdateMain(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    ...
    gAtlasSamples[atlasIndex(cellCoord)] = buildAtlasSample(worldX, worldZ);
}
