...
float4 main(PSInput input) : SV_TARGET
{
    ...
    const float3 directLight = uSunColor.rgb * (diff * shadow * directSun);
    const float3 specularLight = uSunColor.rgb * (spec * shadow * directSun * 0.08f);

    float3 color = textureSample.rgb * (skyIndirect + blockIndirect + uGroundAmbient.rgb * 0.08f + directLight) +
                   specularLight;
