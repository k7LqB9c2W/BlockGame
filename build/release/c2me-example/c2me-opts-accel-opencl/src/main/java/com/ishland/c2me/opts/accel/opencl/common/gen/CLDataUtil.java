...
import com.ishland.c2me.opts.dfc.common.gen.opencl.CLBlockStateMappings;
...
//        int localSize = 0;
        ...
        for (Reference2IntMap.Entry<Object> entry : generatedCLSource.getGlobalDynamicDataOffsets().reference2IntEntrySet()) {
            ...
            if (key == OpenCLGen.MARKER_localOffsetTable) {
            ...
            } else if (key == OpenCLGen.MARKER_cacheLike_cache2d) {
                allocatedOffsets[index] = new OffsetAndData(0, null); // no interpolator
            } else {
...
//                    Assertions.assertTrue(stage1Cache.surfaceHeights().length == OpenCLStructs.SIZE_estimateSurfaceHeight * OpenCLStructs.SIZE_estimateSurfaceHeight);
        ...
        OpenCLStructs.worldgen_params$offset_estimateSurfaceHeight.set(segment, 0L, allocatedOffsets[generatedCLSource.getGlobalDynamicDataOffsets().getInt(OpenCLGen.MARKER_estimateSurfaceHeightCache)].offset);
        ...
        OpenCLStructs.worldgen_params$offset_aquifer.set(segment, 0L, allocatedOffsets[generatedCLSource.getGlobalDynamicDataOffsets().getInt(OpenCLGen.MARKER_aquifer)].offset);
        OpenCLStructs.worldgen_params$offset_fluidLevelSampler.set(segment, 0L, allocatedOffsets[generatedCLSource.getGlobalDynamicDataOffsets().getInt(OpenCLGen.MARKER_fluidLevelSampler)].offset);
        OpenCLStructs.worldgen_params$offset_oreVeinRandom.set(segment, 0L, allocatedOffsets[generatedCLSource.getGlobalDynamicDataOffsets().getInt(OpenCLGen.MARKER_oreVeinRandom)].offset);
    ...
    public static Reference2IntLinkedOpenHashMap<Object> transformGlobalDynamicDataOffsets(Reference2IntLinkedOpenHashMap<Object> globalDynamicDataOffsets) {
        ...
        while (iterator.hasNext()) {
            ...
            if (entry.getKey() instanceof InterpolatedNoiseSampler sampler) {
                ...
                byte[] bytes = OpenCLGen.bytes(sampler);
                newMap.put(new ConstantBlob(bytes, 8), entry.getIntValue());
            } else {
