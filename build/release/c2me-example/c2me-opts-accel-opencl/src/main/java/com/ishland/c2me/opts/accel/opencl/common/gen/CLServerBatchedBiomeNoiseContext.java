...
public class CLServerBatchedBiomeNoiseContext {
    ...
    public static final int BATCH_SIZE = 4; // must be power of 2, determines axis size
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (generatedCLSource.getBiomeMappings() != null) {
                try (MemoryStack _ = MemoryStack.stackPush()) {
                    ...
                    long kernel = CL12.clCreateKernel(deviceWithProgram.getProgram(OpenCLGen.ProgramType.BIOME_MULTINOISE_KERNEL), "df_biome_multinoise_kernel", errorCodeRet);
                    CLUtil.checkCLError(errorCodeRet);
            ...
            try (MemoryStack _ = MemoryStack.stackPush()) {
                ...
                if (generatedCLSource.getInterpolatorPrefills() != 0) {
                    ...
                    long kernel = CL12.clCreateKernel(deviceWithProgram.getProgram(OpenCLGen.ProgramType.INTERPOLATOR_PREFILL), "df_interpolator_buffer_prefill_kernel", errorCodeRet);
                    CLUtil.checkCLError(errorCodeRet);
                    CLUtil.checkCLError(CL12.clSetKernelArg1p(kernel, 0, deviceWithProgram.programConstDataBuffer()));
                    CLUtil.checkCLError(CL12.clSetKernelArg1p(kernel, 1, rwBuffer.buffer()));
            ...
            if (settings.hasAquifers()) {
                try (MemoryStack _ = MemoryStack.stackPush()) {
                    ...
                    PointerBuffer prevEvent = event.get(0) != 0L ? stack.mallocPointer(1).put(0, event.get(0)) : null;
                    long kernel = CL12.clCreateKernel(deviceWithProgram.getProgram(OpenCLGen.ProgramType.AQUIFER_PREFILL), "aquifer_data_prefill", errorCodeRet);
                    CLUtil.checkCLError(errorCodeRet);
                    CLUtil.checkCLError(CL12.clSetKernelArg1p(kernel, 0, deviceWithProgram.programConstDataBuffer()));
                    CLUtil.checkCLError(CL12.clSetKernelArg1p(kernel, 1, rwBuffer.buffer()));
            ...
            if (generatedCLSource.getCache2dPrefills() != 0) {
                try (MemoryStack _ = MemoryStack.stackPush()) {
                    ...
                    PointerBuffer prevEvent = event.get(0) != 0L ? stack.mallocPointer(1).put(0, event.get(0)) : null;
                    long kernel = CL12.clCreateKernel(deviceWithProgram.getProgram(OpenCLGen.ProgramType.CACHE2D_PREFILL), "df_cache2d_prefill_kernel", errorCodeRet);
                    CLUtil.checkCLError(errorCodeRet);
                    CLUtil.checkCLError(CL12.clSetKernelArg1p(kernel, 0, deviceWithProgram.programConstDataBuffer()));
                    CLUtil.checkCLError(CL12.clSetKernelArg1p(kernel, 1, rwBuffer.buffer()));
                    CLUtil.checkCLError(CL12.clEnqueueNDRangeKernel(commandQueue.getCommandQueue(), kernel, 3, null, globalWorkSize, localWorkSize, prevEvent, event));
            ...
            try (MemoryStack _ = MemoryStack.stackPush()) {
                ...
                PointerBuffer prevEvent = event.get(0) != 0L ? stack.mallocPointer(1).put(0, event.get(0)) : null;
                long kernel = CL12.clCreateKernel(deviceWithProgram.getProgram(OpenCLGen.ProgramType.NOISE_KERNEL), "df_noise_kernel", errorCodeRet);
                CLUtil.checkCLError(errorCodeRet);
                CLUtil.checkCLError(CL12.clSetKernelArg1p(kernel, 0, deviceWithProgram.programConstDataBuffer()));
                CLUtil.checkCLError(CL12.clSetKernelArg1p(kernel, 1, rwBuffer.buffer()));
            ...
            CLUtil.checkCLError(CL12.clSetEventCallback(
                    event.get(0),
                    CL12.CL_COMPLETE,
                    ref.callback = CLEventCallback.create((_, event_command_status, _) -> {
