...
public class CLServerWorldContext {
    ...
    public record DeviceWithProgram(OpenCLDevice device, long[] program, long programConstDataBuffer) {
        ...
        public DeviceWithProgram(OpenCLDevice device, EnumMap<OpenCLGen.ProgramType, Long> program, long programConstDataBuffer) {
            this(device, enumMap2Array(program), programConstDataBuffer);
        }

        public long getProgram(OpenCLGen.ProgramType type) {
        ...
        private static long[] enumMap2Array(EnumMap<OpenCLGen.ProgramType, Long> program) {
            long[] array = new long[OpenCLGen.ProgramType.values().length];
            for (OpenCLGen.ProgramType type : OpenCLGen.ProgramType.values()) {
