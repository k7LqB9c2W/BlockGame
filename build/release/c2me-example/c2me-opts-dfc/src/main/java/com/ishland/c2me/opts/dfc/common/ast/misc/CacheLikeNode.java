...
public class CacheLikeNode implements AstNode {
    ...
    public void doBytecodeGenSingle(BytecodeGen.Context context, InstructionAdapter m, BytecodeGen.Context.LocalVarConsumer localVarConsumer) {
        ValuesMethodDefD delegateMethod = context.newSingleMethod(this.delegate);
    ...
    public void doBytecodeGenMulti(BytecodeGen.Context context, InstructionAdapter m, BytecodeGen.Context.LocalVarConsumer localVarConsumer) {
        ValuesMethodDefD delegateMethod = context.newMultiMethod(this.delegate);
        String cacheLikeField = context.newField(IFastCacheLike.class, this.cacheLike);
    ...
    public String doCLGen(OpenCLGen.Context context) {
        if (!((Object) this.cacheLike instanceof DensityFunctionTypes.Wrapping wrapping)) {
            throw new UnsupportedOperationException("Can only gen wrapping");
        }
        ValuesMethodDefD valuesMethodDefD = context.newMethod(this.delegate);
        return switch (wrapping.type()) {
            case CACHE_ONCE, CACHE_ALL_IN_CELL -> "return " + context.callDelegate(valuesMethodDefD) + ";";
            case INTERPOLATED -> {
                int offset = context.getGlobalDynamicDataOffset(OpenCLGen.MARKER_cacheLike_interpolator);
                int ordinal = context.registerInterpolator(this);
            ...
            case FLAT_CACHE -> {
                int offset = context.getGlobalDynamicDataOffset(OpenCLGen.MARKER_cacheLike_flatCache);
                int ordinal = context.registerFlatCache(this);
            ...
            case CACHE2D -> {
                int offset = context.getGlobalDynamicDataOffset(OpenCLGen.MARKER_cacheLike_cache2d);
                int ordinal = context.registerCache2d(this);
...
//                context.registerCacheKernel(this);
//                int ordinal = context.allocGlobalDynamicData(this);
//                yield "if (ctx.rw_data && (ctx.sample_flags & MASK_inInterpolationLoop) && (ctx.sample_flags & MASK_isInterpolation)) {\n" +
//                        "    global const interpolation_parameters_t * restrict params = ctx.rw_data;\n" +
//                        "    global const double * restrict data = df_data_offset_global(ctx.rw_data, " + ordinal + ");\n" +
//                        "    const cache_result_t res = df_cachelike_fullcellcache(params, data, ctx.x, ctx.y, ctx.z, ctx.sample_flags);\n" +
//                        "    if (res.cached) {\n" +
//                        "        return res.res;\n" +
//                        "    }\n" +
//                        "    df_cachelike_trap_printf(\"fullcellcache\", ctx);\n" +
//                        "    __builtin_trap();\n" +
//                        "    __builtin_unreachable();\n" +
//                        "    return nan((uint64_t) 0);\n" +
//                        "}\n" +
