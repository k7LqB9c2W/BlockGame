...
public class DFTShiftBNode implements AstNode {
    ...
    public DFTShiftBNode(DensityFunction.Noise offsetNoise) {
        this.offsetNoise = Objects.requireNonNull(offsetNoise);
    }

    @Override
    public double evalSingle(int x, int y, int z, EvalType type) {
        return this.offsetNoise.sample(z * 0.25, x * 0.25, 0.0) * 4.0;
    }

    @Override
    public void evalMulti(double[] res, int[] x, int[] y, int[] z, EvalType type) {
        for (int i = 0; i < res.length; i++) {
            res[i] = this.offsetNoise.sample(z[i] * 0.25, x[i] * 0.25, 0.0) * 4.0;
        }
    }
    ...
    public String doCLGen(OpenCLGen.Context context) {
        if (this.offsetNoise.noise() == null) {
            ...
            return "global const double_octave_sampler_data_t * restrict data = ptr_shift_global(ctx.const_data, " + offset + ");\n" +
                    ...
                    "(double) ctx.z * 0.25, (double) ctx.x * 0.25, 0.0) * 4.0;\n";
        }
    }
