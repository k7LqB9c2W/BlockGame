...
public class BytecodeGen {
    ...
    public static DensityFunction compile(DensityFunction densityFunction, Reference2ReferenceMap<DensityFunction, AstNode> optoCache, Reference2ReferenceMap<DensityFunction, DensityFunction> tempCache) {
        ...
        AstNode ast = optoCache.computeIfAbsent(densityFunction, (DensityFunction df) -> OptoPasses.optimize(McToAst.toAst(df)));
...
//        }
        ...
        GenDumper.dumpClass(genContext.className, bytes);
...
//    }
    ...
    public static class Context {
        ...
        public String newSingleMethodUnoptimized(AstNode node) {
            return this.singleMethods.computeIfAbsent(node, (AstNode node1) -> this.newSingleMethod((adapter, localVarConsumer) -> node1.doBytecodeGenSingle(this, adapter, localVarConsumer), nextMethodName(node)));
        }
        ...
        public String newMultiMethodUnoptimized(AstNode node) {
            return this.multiMethods.computeIfAbsent(node, (AstNode node1) -> this.newMultiMethod((adapter, localVarConsumer) -> node1.doBytecodeGenMulti(this, adapter, localVarConsumer), nextMethodName(node)));
        }
