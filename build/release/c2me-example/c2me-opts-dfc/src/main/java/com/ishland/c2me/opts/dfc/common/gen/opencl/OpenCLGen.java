...
//    public static final Object MARKER_structureWeightSamplerTable = new Object();
    ...
    public static GeneratedCLSource compile(NoiseRouter noiseRouter, GenerationShapeConfig generationShapeConfig, Reference2ReferenceMap<DensityFunction, AstNode> optoCache, DensityFunction finalFinalDensity, BiomeSource biomeSource) {
        Context context = new Context();
        context.prependConstants(generationShapeConfig);
        context.compileBinding(optimizeCached(noiseRouter.barrierNoise(), optoCache), "barrier");
        context.compileBinding(optimizeCached(noiseRouter.fluidLevelFloodednessNoise(), optoCache), "fluid_level_floodedness");
        context.compileBinding(optimizeCached(noiseRouter.fluidLevelSpreadNoise(), optoCache), "fluid_level_spread");
        context.compileBinding(optimizeCached(noiseRouter.lavaNoise(), optoCache), "lava");
        context.compileBinding(optimizeCached(noiseRouter.temperature(), optoCache), "temperature");
        context.compileBinding(optimizeCached(noiseRouter.vegetation(), optoCache), "vegetation");
        context.compileBinding(optimizeCached(noiseRouter.continents(), optoCache), "continents");
        context.compileBinding(optimizeCached(noiseRouter.erosion(), optoCache), "erosion");
        context.compileBinding(optimizeCached(noiseRouter.depth(), optoCache), "depth");
        context.compileBinding(optimizeCached(noiseRouter.ridges(), optoCache), "ridges");
        context.compileBinding(optimizeCached(noiseRouter.preliminarySurfaceLevel(), optoCache), "preliminary_surface_level");
        context.compileBinding(optimizeCached(noiseRouter.finalDensity(), optoCache), "final_density");
        context.compileBinding(optimizeCached(noiseRouter.veinToggle(), optoCache), "vein_toggle");
        context.compileBinding(optimizeCached(noiseRouter.veinRidged(), optoCache), "vein_ridged");
        context.compileBinding(optimizeCached(noiseRouter.veinGap(), optoCache), "vein_gap");
        context.compileBinding(optimizeCached(finalFinalDensity, optoCache), "final_final_density");
        ...
        Path path = GenDumper.dumpCL("DfcCompiled_" + original.getOrdinal(), original.getGeneratedSource().getBytes(StandardCharsets.UTF_8));
    ...
    private static AstNode optimizeCached(DensityFunction densityFunction, Reference2ReferenceMap<DensityFunction, AstNode> optoCache) {
    ...
    public static class Context {
        ...
        public static final String signature = "(const sample_int32_ctx_t ctx)";
...
//            this.allocGlobalDynamicData(MARKER_structureWeightSamplerTable);
        ...
        public String newMethodUnoptimized(AstNode node) {
            return this.methods.computeIfAbsent(node, (AstNode node1) -> this.newMethod(() -> node1.doCLGen(this), nextMethodName(node.getClass().getSimpleName())));
        }

        private String newMethod(Supplier<String> generator, String name) {
            String functionBody = generator.get();
            this.pendingSource
                    .append("static inline __attribute__((pure)) double ").append(name).append(signature).append(" {\n")
        ...
        public void callDelegate(StringBuilder b, ValuesMethodDefD target) {
            if (target.isConst()) {
                b.append(OpenCLGen.literal(target.constValue()));
            } else {
        ...
        public int allocGlobalConstDataObject(Object obj) {
            byte[] bytes = OpenCLGen.bytesObject(obj);
        ...
        public void genNoiseKernels() {
            ...
            if (!flatCachePrefills.isEmpty()) {
                this.pendingSource
                        ...
                        .append("kernel __attribute__((reqd_work_group_size(16, 16, 1))) void df_flatcache_prefill_kernel(global const void * restrict const const_data, global void * restrict const rw_data, global double * restrict const extra_out) {\n")
                ...
                for (int i = 0, flatCachePrefillsSize = flatCachePrefills.size(); i < flatCachePrefillsSize; i++) {
                    ...
                    this.pendingSource
                            ...
                            .append("    ").append("    ").append(name).append("(const_data, rw_data, extra_out);\n")
                            .append("    ").append("    ").append("return;\n")
                            .append("    ").append("}\n");
//                    this.pendingSource
