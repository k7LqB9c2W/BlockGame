...
struct ChunkBenchmarkMetrics
{
    ...
    void reset() noexcept
    {
        ...
        exactGpuSynthStage.reset();
        exactGpuStampStage.reset();
        exactGpuLightStage.reset();
        exactGpuFaceCountStage.reset();
        exactGpuFacePrefixStage.reset();
        exactGpuAllocateStage.reset();
        exactGpuFaceEmitStage.reset();
        exactGpuTotalStage.reset();
        ...
        exactGpuPageSweepStage.reset();
        exactGpuEmitWaitStage.reset();
        exactGpuEmitFenceLifetimeStage.reset();
        exactGpuWorldgenPageMisses.reset();
        exactGpuEmitMetadataDirtyPages.reset();
        exactGpuEmitMetadataUploadBytes.reset();
        ...
        chunkDependencyWorldgenDeferrals.reset();
        chunkDependencyStructureDeferrals.reset();
        worldgenDependencyReadyToGenerate.reset();
        structureDependencyReadyToGenerate.reset();
        ...
        worldgenDependencyQueueDepth.reset();
        structureDependencyQueueDepth.reset();
    ...
    [[nodiscard]] ChunkBenchmarkReport snapshot() const noexcept
    {
        ...
        report.exactGpuSynthStage = exactGpuSynthStage.snapshot();
        report.exactGpuStampStage = exactGpuStampStage.snapshot();
        report.exactGpuLightStage = exactGpuLightStage.snapshot();
        report.exactGpuFaceCountStage = exactGpuFaceCountStage.snapshot();
        report.exactGpuFacePrefixStage = exactGpuFacePrefixStage.snapshot();
        report.exactGpuAllocateStage = exactGpuAllocateStage.snapshot();
        report.exactGpuFaceEmitStage = exactGpuFaceEmitStage.snapshot();
        report.exactGpuTotalStage = exactGpuTotalStage.snapshot();
        ...
        report.exactGpuPageSweepStage = exactGpuPageSweepStage.snapshot();
        report.exactGpuEmitWaitStage = exactGpuEmitWaitStage.snapshot();
        report.exactGpuEmitFenceLifetimeStage = exactGpuEmitFenceLifetimeStage.snapshot();
        report.exactGpuWorldgenPageMisses = exactGpuWorldgenPageMisses.snapshot();
        report.exactGpuEmitMetadataDirtyPages = exactGpuEmitMetadataDirtyPages.snapshot();
        report.exactGpuEmitMetadataUploadBytes = exactGpuEmitMetadataUploadBytes.snapshot();
        ...
        report.chunkDependencyWorldgenDeferrals = chunkDependencyWorldgenDeferrals.snapshot();
        report.chunkDependencyStructureDeferrals = chunkDependencyStructureDeferrals.snapshot();
        report.worldgenDependencyReadyToGenerate = worldgenDependencyReadyToGenerate.snapshot();
        report.structureDependencyReadyToGenerate = structureDependencyReadyToGenerate.snapshot();
        ...
        report.worldgenDependencyQueueDepth = worldgenDependencyQueueDepth.snapshot();
        report.structureDependencyQueueDepth = structureDependencyQueueDepth.snapshot();
    ...
    AtomicLatencyHistogram exactGpuSynthStage{};
    AtomicLatencyHistogram exactGpuStampStage{};
    AtomicLatencyHistogram exactGpuLightStage{};
    AtomicLatencyHistogram exactGpuFaceCountStage{};
    AtomicLatencyHistogram exactGpuFacePrefixStage{};
    AtomicLatencyHistogram exactGpuAllocateStage{};
    AtomicLatencyHistogram exactGpuFaceEmitStage{};
    AtomicLatencyHistogram exactGpuTotalStage{};
    ...
    AtomicLatencyHistogram exactGpuPageSweepStage{};
    AtomicLatencyHistogram exactGpuEmitWaitStage{};
    AtomicLatencyHistogram exactGpuEmitFenceLifetimeStage{};
    AtomicCountHistogram exactGpuWorldgenPageMisses{};
    AtomicCountHistogram exactGpuEmitMetadataDirtyPages{};
    AtomicCountHistogram exactGpuEmitMetadataUploadBytes{};
    ...
    AtomicCountHistogram chunkDependencyWorldgenDeferrals{};
    AtomicCountHistogram chunkDependencyStructureDeferrals{};
    AtomicLatencyHistogram worldgenDependencyReadyToGenerate{};
    AtomicLatencyHistogram structureDependencyReadyToGenerate{};
    ...
    AtomicDepthHistogram<4096> worldgenDependencyQueueDepth{};
    AtomicDepthHistogram<4096> structureDependencyQueueDepth{};
...
[[nodiscard]] int preloadExactDrawRadiusChunks(int exactDrawRadius,
                                               bool startupEnabled,
                                               StreamingPhase phase) noexcept
{
    (void)startupEnabled;
    (void)phase;
    return exactDrawRadius;
}
                                                         ...
                                                         const char* target)
{
    ...
    return bytecode;
#else
    const auto compileWithDxc = [&](const std::wstring& widePath) -> Microsoft::WRL::ComPtr<ID3DBlob>
    {
        const auto findDxcExecutable = []() -> std::filesystem::path
        {
            if (const char* envValue = std::getenv("BLOCKGAME_DXC_EXE");
            ...
            const std::filesystem::path bundledPath = "D:/dxc_2026_02_20/bin/x64/dxc.exe";
            ...
            const DWORD resolvedLength =
                SearchPathW(nullptr, L"dxc.exe", nullptr, static_cast<DWORD>(std::size(resolvedPath)), resolvedPath, nullptr);
            if (resolvedLength != 0 && resolvedLength < std::size(resolvedPath))
            {
                return resolvedPath;
            }
        ...
        const std::filesystem::path dxcPath = findDxcExecutable();
        if (dxcPath.empty())
        {
            throw std::runtime_error("DXC is required to compile shader model 6.x shaders at runtime");
        }

        const std::filesystem::path sourcePath = std::filesystem::path(widePath);
        const std::filesystem::path outputDirectory =
            std::filesystem::temp_directory_path() / "blockgame_runtime_shader_cache";
        std::filesystem::create_directories(outputDirectory);
        const std::filesystem::path outputPath =
            outputDirectory / compiledShaderFileName(sourcePath.filename().string(), entryPoint, target);

        std::vector<std::wstring> storage;
        storage.reserve(14);
        storage.push_back(dxcPath.wstring());
        storage.push_back(L"-E");
        storage.push_back(std::wstring(entryPoint, entryPoint + std::strlen(entryPoint)));
        storage.push_back(L"-T");
        storage.push_back(std::wstring(target, target + std::strlen(target)));
        storage.push_back(L"-Fo");
        storage.push_back(outputPath.wstring());
        storage.push_back(L"-I");
        storage.push_back(sourcePath.parent_path().wstring());
        storage.push_back(L"-Zpr");
#ifndef NDEBUG
        storage.push_back(L"-Od");
        storage.push_back(L"-Zi");
#endif
        storage.push_back(sourcePath.wstring());
        ...
        const intptr_t spawnResult = _wspawnv(_P_WAIT, dxcPath.c_str(), argv.data());
        if (spawnResult != 0)
        {
            throw std::runtime_error("DXC shader compilation failed for " + path);
        }

        Microsoft::WRL::ComPtr<ID3DBlob> bytecode;
        throwIfFailedDx(D3DReadFileToBlob(outputPath.wstring().c_str(), &bytecode),
                        "failed to load DXC-compiled shader bytecode");
        return bytecode;
    };

    const std::wstring widePath = std::filesystem::path(path).wstring();
    if (std::string_view(target).starts_with("cs_6_"))
    {
        return compileWithDxc(widePath);
    }
...
struct GpuExactColumnDescriptor
{
    ...
    std::uint32_t stripePeriod{0};
    std::uint32_t stripeThickness{0};
    std::uint32_t grassTintIndex{0};
...
static_assert(sizeof(GpuExactColumnDescriptor) == 64u);

struct GpuExactSparseVoxel
{
    std::uint32_t packedLocalPos{0};
...
struct GpuExactPrepassRecord
{
    ...
    std::uint32_t scratchSliceIndex{0};
    std::uint32_t descriptorOffset{0};
    std::uint32_t rebuildVoxelInputs{0};
    std::uint32_t sparseVoxelCount{0};
    std::uint32_t resolvedNeighborMask{0};
    std::uint32_t closedNeighborMask{0};
    std::uint32_t pendingNeighborMask{0};
    std::uint32_t centerVoxelSrvDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t centerVoxelUavDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t haloVoxelSrvDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t haloVoxelUavDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t lightScratchVoxelSrvDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t lightScratchVoxelUavDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t sparseVoxelSrvDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t neighborPosXVoxelSrvDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t neighborNegXVoxelSrvDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t neighborPosYVoxelSrvDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t neighborNegYVoxelSrvDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t neighborPosZVoxelSrvDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t neighborNegZVoxelSrvDescriptorIndex{FarLodGpuContext::kInvalidDescriptorIndex};
    std::uint32_t reserved0{0};
};
static_assert(sizeof(GpuExactPrepassRecord) == 96u);
...
struct Chunk
{
    ...
    void reset(const glm::ivec3& c)
    {
        ...
        exactGpuBuildVersion.store(0, std::memory_order_relaxed);
    ...
    bool gpuExactSparseVoxelsReady{false};
    ...
    std::atomic<std::uint32_t> exactGpuBuildVersion{0};
...
[[nodiscard]] std::uint32_t packGpuExactLocalVoxelPos(const glm::ivec3& localPos) noexcept
{
    const std::uint32_t x = static_cast<std::uint32_t>(std::clamp(localPos.x, 0, kChunkSizeX - 1));
    const std::uint32_t y = static_cast<std::uint32_t>(std::clamp(localPos.y, 0, kChunkSizeY - 1));
    const std::uint32_t z = static_cast<std::uint32_t>(std::clamp(localPos.z, 0, kChunkSizeZ - 1));
    return x | (y << 5u) | (z << 10u);
}
...
inline constexpr std::uint32_t kInvalidExactGpuDrawRecordIndex = std::numeric_limits<std::uint32_t>::max();
inline constexpr std::uint32_t kExactChunkSizeX = kChunkSizeX;
inline constexpr std::uint32_t kExactChunkSizeY = kChunkSizeY;
...
inline constexpr std::size_t kMaxOutstandingExactGpuBatches = 8u;
...
inline constexpr std::uint8_t kExactGpuSeamPosYBit = 1u << 2u;
inline constexpr std::uint8_t kExactGpuSeamNegYBit = 1u << 3u;
inline constexpr std::uint8_t kExactGpuSeamPosZBit = 1u << 4u;
inline constexpr std::uint8_t kExactGpuSeamNegZBit = 1u << 5u;
inline constexpr std::uint32_t kExactCompletionStatusCompletedBit = 1u << 0u;
...
[[nodiscard]] constexpr std::uint8_t oppositeExactGpuSeamBit(std::uint8_t seamBit) noexcept
{
    switch (seamBit)
    {
    case kExactGpuSeamPosXBit: return kExactGpuSeamNegXBit;
    case kExactGpuSeamNegXBit: return kExactGpuSeamPosXBit;
    case kExactGpuSeamPosYBit: return kExactGpuSeamNegYBit;
    case kExactGpuSeamNegYBit: return kExactGpuSeamPosYBit;
    case kExactGpuSeamPosZBit: return kExactGpuSeamNegZBit;
...
private:
    ...
    struct PendingExactGpuBatch
    {
        ...
        SteadyClock::time_point prepassReadyForEmitAt{};
        SteadyClock::time_point emitSubmittedAt{};
    ...
    static constexpr std::size_t kExactReservationShardCount = 1u;
    static constexpr std::size_t kExactReservationShardSoftOpenPageLimit = 2u;
    ...
    [[nodiscard]] bool reserveExactOutputRange(const glm::ivec3& chunkCoord,
                                               ...
                                               ChunkAllocation& allocation,
                                               std::uint32_t& reservedFaceCapacity);
    void requestSealExactPage(std::uint32_t pageIndex);
    void collectSealedExactPages(UINT64 completedGpuFenceValue);
    ...
    void queueChunkForExactGpuBuild(const std::shared_ptr<Chunk>& chunk, std::uint32_t buildVersion);
    ...
    ColumnSample sampleColumnBlocking(int worldX,
                                      int worldZ,
                                      int slabMinWorldY = std::numeric_limits<int>::min(),
                                      int slabMaxWorldY = std::numeric_limits<int>::max(),
                                      bool includeBlendDebug = false) const;
    bool trySampleColumnReady(ColumnSample& outSample,
                              int worldX,
                              int worldZ,
                              int slabMinWorldY = std::numeric_limits<int>::min(),
                              int slabMaxWorldY = std::numeric_limits<int>::max(),
    ...
    {
        bool operator()(const WorldgenPageDependencyRequest& lhs,
                        const WorldgenPageDependencyRequest& rhs) const noexcept
        {
    ...
    [[nodiscard]] std::size_t worldgenPageDependencyQueueLimit(DependencyPriority priority,
    ...
    void registerDeferredChunkDependencies(const glm::ivec3& coord,
                                           std::uint32_t generationEpoch,
                                           bool initialReadyPriority,
                                           bool localInteractionPriority,
                                           std::span<const glm::ivec2> missingWorldgenPages,
    ...
    std::array<ExactReservationShard, kExactReservationShardCount> exactReservationShards_{};
    ...
    Microsoft::WRL::ComPtr<ID3D12Resource> exactGpuChunkAllocationRecordBuffer_{};
    D3D12_RESOURCE_STATES exactGpuEmitConfigBufferState_{D3D12_RESOURCE_STATE_COMMON};
    D3D12_RESOURCE_STATES exactGpuPageMetadataBufferState_{D3D12_RESOURCE_STATE_COMMON};
    D3D12_RESOURCE_STATES exactGpuChunkAllocationRecordBufferState_{D3D12_RESOURCE_STATE_COMMON};
    std::uint32_t exactGpuPageMetadataCapacity_{0};
    std::uint32_t exactGpuChunkAllocationRecordCapacity_{0};
    FarLodGpuContext::ExactEmitConfig exactGpuEmitConfig_{};
    std::vector<std::uint32_t> exactGpuDirtyPageIndices_{};
    std::vector<std::uint8_t> exactGpuPageMetadataQueued_{};
    bool exactGpuEmitConfigDirty_{true};
    ...
    std::uint32_t exactGpuBlockUvSrvDescriptorIndex_{FarLodGpuContext::kInvalidDescriptorIndex};
    Microsoft::WRL::ComPtr<ID3D12Resource> exactGpuEmptyVoxelBuffer_;
    D3D12_RESOURCE_STATES exactGpuBlockUvBufferState_{D3D12_RESOURCE_STATE_COMMON};
    D3D12_RESOURCE_STATES exactGpuEmptyVoxelBufferState_{D3D12_RESOURCE_STATE_COMMON};
    std::atomic<int> exactGpuBuildReadbackFailures_{0};
    ...
    double exactGpuWorldgenResolveMsLastCycle_{0.0};
    double exactGpuPrepassFaceTotalsReadbackMsLastCycle_{0.0};
    double exactGpuEmitMetadataSyncMsLastCycle_{0.0};
    ...
    int exactGpuEmitMetadataDirtyPagesLastCycle_{0};
    ...
    std::size_t exactGpuEmitMetadataUploadBytesLastCycle_{0u};
    std::vector<std::uint32_t> freeExactGpuScratchSlices_{};
...
ChunkManager::Impl::~Impl()
{
    ...
    clear();
    destroyBufferPages();
    freeExactGpuScratchSlices_.clear();
...
void ChunkManager::Impl::initializeRendering(ID3D12Device* device)
{
    ...
    resetExactGpuEmitBuffers();
    exactGpuChunkAllocationRecords_.assign(exactGpuContext_.maxExactGpuBuildBatches(), {});
    ...
    freeExactGpuScratchSlices_.clear();
    freeExactGpuScratchSlices_.reserve(exactGpuContext_.maxExactGpuBuildBatches());
    for (std::uint32_t sliceIndex = 0; sliceIndex < exactGpuContext_.maxExactGpuBuildBatches(); ++sliceIndex)
    {
        freeExactGpuScratchSlices_.push_back(sliceIndex);
    }
...
void ChunkManager::Impl::rebuildExactGpuBlockUvBuffer()
{
    ...
    exactGpuBlockUvBufferState_ = D3D12_RESOURCE_STATE_COMMON;
    if (device_ == nullptr || !blockAtlasConfigured_)
    {
        exactGpuBlockUvSrvDescriptorIndex_ = FarLodGpuContext::kInvalidDescriptorIndex;
    ...
    const std::uint64_t bufferBytes =
        static_cast<std::uint64_t>(uvTable.size() * sizeof(FarTerrainManager::GpuBlockFaceUv));
    exactGpuBlockUvBuffer_ = createDefaultBuffer(device_.Get(), bufferBytes, D3D12_RESOURCE_STATE_COMMON);
    if (exactGpuBlockUvBuffer_ == nullptr)
    {
        return;
    }
    ...
    Microsoft::WRL::ComPtr<ID3D12Resource> upload =
        createUploadBuffer(device_.Get(), bufferBytes, uploadMapped);
    ...
    std::memcpy(uploadMapped, uvTable.data(), static_cast<std::size_t>(bufferBytes));
    if (uploadContext_.ready() && uploadContext_.begin())
    {
        uploadContext_.transition(exactGpuBlockUvBuffer_.Get(),
                                  D3D12_RESOURCE_STATE_COMMON,
                                  D3D12_RESOURCE_STATE_COPY_DEST);
        uploadContext_.copyBuffer(exactGpuBlockUvBuffer_.Get(), 0, upload.Get(), 0, bufferBytes);
        uploadContext_.transition(exactGpuBlockUvBuffer_.Get(),
                                  D3D12_RESOURCE_STATE_COPY_DEST,
                                  D3D12_RESOURCE_STATE_COMMON);
    ...
    exactGpuBlockUvCount_ = static_cast<std::uint32_t>(uvTable.size());
    if (exactGpuBlockUvSrvDescriptorIndex_ == FarLodGpuContext::kInvalidDescriptorIndex)
    {
        exactGpuBlockUvSrvDescriptorIndex_ = exactGpuContext_.allocatePersistentDescriptorRange(1u);
    }
    exactGpuContext_.writePersistentStructuredSrvDescriptor(
        exactGpuBlockUvSrvDescriptorIndex_,
        exactGpuBlockUvBuffer_.Get(),
        0,
        exactGpuBlockUvCount_,
        static_cast<std::uint32_t>(sizeof(FarTerrainManager::GpuBlockFaceUv)));
    exactGpuEmitConfig_.blockFaceUvDescriptorIndex = exactGpuBlockUvSrvDescriptorIndex_;
    noteExactGpuEmitConfigRefresh();
}

void ChunkManager::Impl::ensureExactGpuEmptyVoxelBuffer()
{
    exactGpuEmptyVoxelBuffer_.Reset();
    exactGpuEmptyVoxelBufferState_ = D3D12_RESOURCE_STATE_COMMON;
    if (device_ == nullptr)
    {
        return;
    }
    ...
    exactGpuEmptyVoxelBuffer_ =
        createDefaultBuffer(device_.Get(), bufferBytes, D3D12_RESOURCE_STATE_COMMON);
    if (exactGpuEmptyVoxelBuffer_ == nullptr)
    {
        return;
    }
    ...
    if (uploadContext_.ready() && uploadContext_.begin())
    {
        uploadContext_.transition(exactGpuEmptyVoxelBuffer_.Get(),
                                  D3D12_RESOURCE_STATE_COMMON,
                                  D3D12_RESOURCE_STATE_COPY_DEST);
        uploadContext_.copyBuffer(exactGpuEmptyVoxelBuffer_.Get(), 0, upload.Get(), 0, bufferBytes);
        uploadContext_.transition(exactGpuEmptyVoxelBuffer_.Get(),
                                  D3D12_RESOURCE_STATE_COPY_DEST,
                                  D3D12_RESOURCE_STATE_COMMON);
        uploadContext_.flush(nullptr);
...
bool ChunkManager::Impl::ensureExactGpuFreePageReserve(std::size_t minFreePageCount)
{
    ...
    std::size_t freeExactPageCount = 0u;
    ...
    while (freeExactPageCount < minFreePageCount)
    {
        ChunkBufferPage reservePage{};
        try
        {
            reservePage = createBufferPage(1u, 1u, ChunkBufferPageUsage::ExactGpu);
        }
        ...
        bufferPages_.push_back(std::move(reservePage));
        noteExactGpuEmitConfigRefresh();
        noteExactGpuPageMetadataChange(static_cast<std::uint32_t>(bufferPages_.size() - 1u));
        ++freeExactPageCount;
    }
...
bool ChunkManager::Impl::ensureChunkExactGpuBuildResourcesLocked(Chunk& chunk)
{
    if (device_ == nullptr || !exactGpuContext_.ready())
    {
        return false;
    ...
    if (sparseVoxelCount > 0 &&
        (chunk.exactGpu.sparseVoxelBuffer == nullptr ||
         chunk.exactGpu.sparseVoxelCapacity < sparseVoxelCount))
    {
        const std::uint64_t sparseBytes =
            static_cast<std::uint64_t>(std::max<std::uint32_t>(sparseVoxelCount, 1u)) *
            sizeof(GpuExactSparseVoxel);
        chunk.exactGpu.sparseVoxelBuffer =
            createDefaultBuffer(device_.Get(), sparseBytes, D3D12_RESOURCE_STATE_COMMON);
    ...
    {
        chunk.exactGpu.voxelBuffer = createDefaultBuffer(device_.Get(),
                                                         voxelBytes,
                                                         D3D12_RESOURCE_STATE_COMMON,
    ...
    return chunk.exactGpu.voxelBuffer != nullptr &&
...
ID3D12Fence* ChunkManager::Impl::exactGpuFence() const noexcept
{
    return exactGpuContext_.fence();
}

std::uint64_t ChunkManager::Impl::lastSubmittedExactGpuFenceValue() const noexcept
{
    return static_cast<std::uint64_t>(exactGpuContext_.lastSubmittedFenceValue());
}
...
void ChunkManager::Impl::update(const glm::vec3& cameraPos, const glm::vec3& cameraForward)
{
    ...
    if (renderSettings_.totalChunks > renderSettings_.exactChunks)
    {
        const std::size_t exactPendingUploads = estimatePendingExactGpuBuilds();
        ...
        farTerrainManager_.update(centerChunk,
                                  ...
                                  {
                                      return sampleColumnBlocking(worldX, worldZ, slabMinWorldY, slabMaxWorldY);
                                  });
    }
    else
    {
    ...
    if (benchmarkEnabled)
    {
        ...
        benchmarkMetrics_.uploadQueueDepth.record(estimateUploadQueueSize());
        {
            ...
            benchmarkMetrics_.worldgenDependencyQueueDepth.record(
                static_cast<std::uint64_t>(pendingWorldgenPageDependencyQueue_.size()));
        }
        {
...
WorldRenderData ChunkManager::Impl::buildRenderData(const Frustum& frustum) const
{
    ...
    const int exactDrawRadiusChunks = preloadExactDrawRadiusChunks(
    ...
    std::vector<bool> exactGpuResidentPages;
    {
        ...
        for (std::size_t i = 0; i < pageCount; ++i)
        {
            ...
            if (bufferPages_[i].usage == ChunkBufferPageUsage::ExactGpu)
            {
                ...
                if (!residentExactPage)
                {
                    renderData.nearBatches[i].gpuCullRecordBuffer = nullptr;
                    renderData.nearBatches[i].gpuCullRecordCount = 0u;
                    renderData.nearBatches[i].supportsGpuCull = false;
                }
            }
        }
    }
    ...
    for (const auto& [coord, chunkPtr] : snapshot)
    {
        ...
        const bool exactGpuResident = chunkPtr->exactGpuResident.load(std::memory_order_acquire);
        ...
        if (!exactGpuResident && indexCount == 0)
        {
            continue;
        }
    ...
    auto emptyIt = std::remove_if(renderData.nearBatches.begin(),
                                  ...
                                  [&exactGpuPageVisible](const ChunkRenderBatch& batch)
                                  {
                                      ...
                                      return batch.indexCounts.empty() &&
                                             (!exactGpuVisible || batch.gpuCullRecordCount == 0);
                                  });
...
float ChunkManager::Impl::surfaceHeight(float worldX, float worldZ) const noexcept
{
    ...
    const ColumnSample sample = sampleColumnBlocking(wx, wz);
                                                ...
                                                int slabMaxWorldY) const
{
    ...
    return sampleColumnBlocking(worldX, worldZ, slabMinWorldY, slabMaxWorldY, true);
}
...
void ChunkManager::Impl::clear()
{
    ...
    clearStreamingChunkLifecycleStates();
    {
        ...
        pendingWorldgenPageDependencyQueue_ = {};
...
glm::vec3 ChunkManager::Impl::findSafeSpawnPosition(float worldX, float worldZ) const
{
    ...
    const ColumnSample baseSample = sampleColumnBlocking(baseX, baseZ);
    auto sampleColumnAt = [&](int worldX, int worldZ) -> ColumnSample
    {
        return sampleColumnBlocking(worldX, worldZ);
    };
    ...
    auto predictTreeCanopyTop = [&](int originX, int originZ, const ColumnSample& columnSample, int targetX, int targetZ) -> int
    {
        ...
        if (terrain::isTaigaBiome(biome))
        {
            ...
            for (int trunkX = 0; trunkX < 2; ++trunkX)
            {
                for (int trunkZ = 0; trunkZ < 2; ++trunkZ)
                {
                    const ColumnSample trunkSample = sampleColumnBlocking(originX + trunkX, originZ + trunkZ);
            ...
            for (int dx = -2; dx <= 3; ++dx)
            {
                for (int dz = -2; dz <= 3; ++dz)
                {
                    const ColumnSample neighborSample = sampleColumnBlocking(originX + dx, originZ + dz);
                    if (!neighborSample.dominantBiome)
                    {
                        return ColumnManager::kNoHeight;
                    }
    ...
    for (int originX = baseX - kTaigaSpruceMaxLeafRadius; originX <= baseX + kTaigaSpruceMaxLeafRadius; ++originX)
    {
        for (int originZ = baseZ - kTaigaSpruceMaxLeafRadius; originZ <= baseZ + kTaigaSpruceMaxLeafRadius; ++originZ)
        {
            const ColumnSample originSample =
                (originX == baseX && originZ == baseZ) ? baseSample : sampleColumnBlocking(originX, originZ);
...
StreamingStatusSnapshot ChunkManager::Impl::computeStreamingStatusSnapshot() const noexcept
{
    ...
    {
        snapshot.blockingReason = "waiting for exact gpu builds";
    }
    else
    {
...
ChunkProfilingSnapshot ChunkManager::Impl::sampleProfilingSnapshot()
{
        ...
        static_cast<std::size_t>(std::numeric_limits<int>::max())));
    {
        ...
        snapshot.columnPrefetchQueueDepth = static_cast<int>(std::min<std::size_t>(
            pendingWorldgenPageDependencyQueue_.size(),
            static_cast<std::size_t>(std::numeric_limits<int>::max())));
    }
    ...
    snapshot.exactGpuBuildReadbackFailures =
        exactGpuBuildReadbackFailures_.exchange(0, std::memory_order_relaxed);
    snapshot.exactGpuBuildResourceFailures =
        exactGpuBuildResourceFailures_.exchange(0, std::memory_order_relaxed);
    snapshot.exactGpuBuildOverflows =
        exactGpuBuildOverflows_.exchange(0, std::memory_order_relaxed);
    snapshot.exactGpuBuildStaleCancels =
        exactGpuBuildStaleCancels_.exchange(0, std::memory_order_relaxed);
    snapshot.exactGpuBuildsSubmitted =
        exactGpuBuildsSubmitted_.load(std::memory_order_relaxed);
    snapshot.exactGpuBuildsCommitted =
        ...
        exactGpuMeshReplacements_.load(std::memory_order_relaxed);
    {
        ...
        snapshot.exactGpuWorldgenResolveMsLastCycle = exactGpuWorldgenResolveMsLastCycle_;
        snapshot.exactGpuPrepassFaceTotalsReadbackMsLastCycle = exactGpuPrepassFaceTotalsReadbackMsLastCycle_;
        snapshot.exactGpuEmitMetadataSyncMsLastCycle = exactGpuEmitMetadataSyncMsLastCycle_;
        snapshot.exactGpuPageSweepMsLastCycle = exactGpuPageSweepMsLastCycle_;
        snapshot.exactGpuEmitWaitMsLastCycle = exactGpuEmitWaitMsLastCycle_;
        ...
        snapshot.exactGpuAllocateMs = lastExactGpuPassTimings_.allocateMs;
        snapshot.exactGpuFaceEmitMs = lastExactGpuPassTimings_.faceEmitMs;
        snapshot.exactGpuTotalMs = lastExactGpuPassTimings_.totalMs;
        snapshot.exactGpuWorldgenPageMissesLastCycle = exactGpuWorldgenPageMissesLastCycle_;
        snapshot.exactGpuEmitMetadataDirtyPagesLastCycle = exactGpuEmitMetadataDirtyPagesLastCycle_;
        ...
        snapshot.exactGpuEmitMetadataUploadBytesLastCycle = exactGpuEmitMetadataUploadBytesLastCycle_;
...
void ChunkManager::Impl::resetBenchmarkMetrics()
{
    ...
    exactGpuMeshReplacements_.store(0, std::memory_order_relaxed);
    {
        ...
        exactGpuWorldgenResolveMsLastCycle_ = 0.0;
        exactGpuPrepassFaceTotalsReadbackMsLastCycle_ = 0.0;
        exactGpuEmitMetadataSyncMsLastCycle_ = 0.0;
        ...
        exactGpuEmitMetadataDirtyPagesLastCycle_ = 0;
        ...
        exactGpuEmitMetadataUploadBytesLastCycle_ = 0u;
    }
    if (climateMap_)
    {
...
std::string ChunkManager::Impl::biomeNameAt(const glm::vec3& worldPos) const
{
    const int worldX = static_cast<int>(std::floor(worldPos.x));
    const int worldZ = static_cast<int>(std::floor(worldPos.z));
    const ColumnSample sample = sampleColumnBlocking(worldX, worldZ);
    if (sample.dominantBiome)
    {
...
void ChunkManager::Impl::cancelPendingExactGpuBuildsForShutdown()
{
    ...
    std::vector<PendingExactGpuBuild> pendingBuilds;
    {
        ...
        for (PendingExactGpuBatch& batch : pendingExactGpuBatches_)
        {
            if (batch.prepassResources.submissionSlot != std::numeric_limits<std::uint32_t>::max())
            {
                exactGpuContext_.releaseSubmissionSlot(batch.prepassResources.submissionSlot);
            }
            if (batch.emitResources.submissionSlot != std::numeric_limits<std::uint32_t>::max())
            {
                exactGpuContext_.releaseSubmissionSlot(batch.emitResources.submissionSlot);
            }
            for (PendingExactGpuBuild& build : batch.builds)
            {
                freeExactGpuScratchSlices_.push_back(build.scratchSliceIndex);
                pendingBuilds.push_back(std::move(build));
            }
        }
        pendingExactGpuBatches_.clear();
    }

    for (PendingExactGpuBuild& pending : pendingBuilds)
    {
        releasePendingExactBuildAllocation(pending, true);
           ...
           !exactGpuWorkerStopRequested_.load(std::memory_order_acquire))
    {
        ...
        bool canMaintainExactGpuReserve = true;
        {
            ...
            canMaintainExactGpuReserve =
                std::none_of(pendingExactGpuBatches_.begin(),
                             pendingExactGpuBatches_.end(),
                             [](const PendingExactGpuBatch& batch)
                             {
                                 return batch.phase == ExactGpuBatchPhase::EmitSubmitted &&
...
{
    if (job.type == JobType::WorldgenPageDependency)
    {
        (void)processWorldgenPageDependencyJob();
        refillWorldgenPageDependencyJobs();
        return;
    }
    ...
    {
        struct GenerateJobGuard
        {
            bool previousValue{false};

            GenerateJobGuard()
                : previousValue(gChunkManagerInsideGenerateJob)
            {
                gChunkManagerInsideGenerateJob = true;
            }

            ~GenerateJobGuard()
            {
                gChunkManagerInsideGenerateJob = previousValue;
            }
        } generateJobGuard{};
        ...
        if (benchmarkEnabled)
        {
            ...
            recordCount(benchmarkMetrics_.chunkReadyRequestQueuedPrefetchStage,
                        chunk->requestQueuedByType[jobTypeIndex(JobType::WorldgenPageDependency)]);
            ...
            benchmarkMetrics_.chunkReadyStartQueuedPrefetchStage.record(
                static_cast<std::uint64_t>(queueSnapshot.queuedByType[jobTypeIndex(JobType::WorldgenPageDependency)]));
            ...
            benchmarkMetrics_.chunkReadyStartActivePrefetchStage.record(
                static_cast<std::uint64_t>(queueSnapshot.activeByType[jobTypeIndex(JobType::WorldgenPageDependency)]));
    ...
    const ChunkBufferPage& page) noexcept
{
    ...
    metadata.vertexUavDescriptorIndex = page.exactVertexUavDescriptorIndex;
    metadata.indexUavDescriptorIndex = page.exactIndexUavDescriptorIndex;
                                                             ...
                                                             std::uint32_t haloSrvDescriptorIndex) noexcept
{
    ...
    record.vertexBase = static_cast<std::uint32_t>(pending.allocation.vertexOffset);
    record.indexBase = static_cast<std::uint32_t>(pending.allocation.indexOffset);
                                                                         ...
                                                                         ChunkBufferPageUsage usage)
{
    ...
    {
        const std::size_t minReservedVertexCount =
            static_cast<std::size_t>(kExactGpuInitialFaceCapacity) * 4u;
        const std::size_t minReservedIndexCount =
            static_cast<std::size_t>(kExactGpuInitialFaceCapacity) * 6u;
        page.recordCapacity = std::max<std::size_t>(
            1,
            std::min(page.vertexCapacity / minReservedVertexCount,
                     page.indexCapacity / minReservedIndexCount));
    }
    else
    {
        page.recordCapacity = std::max<std::size_t>(
    ...
    if (usage == ChunkBufferPageUsage::ExactGpu)
    {
        page.drawRecordMetadataBuffer = createDefaultBuffer(
            ...
            D3D12_RESOURCE_STATE_COMMON,
            D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
        page.exactVertexUavDescriptorIndex = exactGpuContext_.allocatePersistentDescriptorRange(4u);
        page.exactIndexUavDescriptorIndex = page.exactVertexUavDescriptorIndex + 1u;
        page.exactDrawRecordUavDescriptorIndex = page.exactVertexUavDescriptorIndex + 2u;
        page.exactDrawRecordMetadataUavDescriptorIndex = page.exactVertexUavDescriptorIndex + 3u;
        exactGpuContext_.writePersistentStructuredUavDescriptor(
            page.exactVertexUavDescriptorIndex,
            page.vertexBuffer.Get(),
            0,
            static_cast<std::uint32_t>(page.vertexCapacity),
            static_cast<std::uint32_t>(sizeof(Vertex)));
        exactGpuContext_.writePersistentStructuredUavDescriptor(
            page.exactIndexUavDescriptorIndex,
    ...
    if (exactUploadDebugLoggingEnabled())
    {
        ...
        stream << "exact upload page created"
               ...
               << (static_cast<double>(vertexBytes + indexBytes + recordBytes + metadataBytes +
    ...
    for (ExactReservationShard& shard : exactReservationShards_)
    {
        ...
        if (openPages.size() > kExactReservationShardSoftOpenPageLimit)
        {
            ...
            for (std::size_t i = 0; i < excessCount; ++i)
            {
                ...
                {
                    bufferPages_[openPages[i]].sealRequested = true;
                }
            }
        }
    }
...
void ChunkManager::Impl::transitionExactPageToWritable(std::uint32_t pageIndex, ChunkBufferPage& page)
{
    ...
    if (page.vertexBuffer != nullptr)
    {
        exactGpuContext_.transition(page.vertexBuffer.Get(), page.vertexState, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        page.vertexState = D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
    }
    if (page.indexBuffer != nullptr)
    {
        exactGpuContext_.transition(page.indexBuffer.Get(), page.indexState, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
...
void ChunkManager::Impl::transitionExactPageToResident(std::uint32_t pageIndex, ChunkBufferPage& page)
{
    ...
    if (page.vertexBuffer != nullptr)
    {
        exactGpuContext_.transition(page.vertexBuffer.Get(), page.vertexState, D3D12_RESOURCE_STATE_COMMON);
        page.vertexState = D3D12_RESOURCE_STATE_COMMON;
    }
    if (page.indexBuffer != nullptr)
    {
        exactGpuContext_.transition(page.indexBuffer.Get(), page.indexState, D3D12_RESOURCE_STATE_COMMON);
        page.indexState = D3D12_RESOURCE_STATE_COMMON;
    }
    if (page.drawRecordBuffer != nullptr)
    {
                                                                                        ...
                                                                                        std::uint32_t& pageIndex)
{
    ...
    for (std::uint32_t candidateIndex = 0; candidateIndex < bufferPages_.size(); ++candidateIndex)
    {
        ...
        if (page.usage != ChunkBufferPageUsage::ExactGpu ||
            page.state != ChunkBufferPageState::Free ||
            page.vertexCapacity < minVertexCount ||
            page.indexCapacity < minIndexCount ||
                                                 ...
                                                 std::uint32_t& reservedFaceCapacity)
{
    ...
    std::lock_guard<std::mutex> shardLock(shard.mutex);
    const std::size_t requiredVertexCount = static_cast<std::size_t>(requiredFaceCount) * 4u;
    const std::size_t requiredIndexCount = static_cast<std::size_t>(requiredFaceCount) * 6u;
    ...
    while (attemptsRemaining-- > 0u)
    {
        ...
        if (openPage->vertexCursor + requiredVertexCount > openPage->vertexCapacity ||
            openPage->indexCursor + requiredIndexCount > openPage->indexCapacity ||
            openPage->recordCursor + 1u > openPage->recordCapacity)
        {
            requestSealExactPage(openPageIndex);
            continue;
        }

        allocation.pageIndex = openPageIndex;
        allocation.vertexOffset = openPage->vertexCursor;
        allocation.indexOffset = openPage->indexCursor;
        ...
        openPage->vertexCursor += requiredVertexCount;
        openPage->indexCursor += requiredIndexCount;
        ++openPage->recordCursor;
        ++openPage->pendingChunks;
        openPage->lastReservationAt = SteadyClock::now();
        if (openPage->vertexCursor == openPage->vertexCapacity ||
            openPage->indexCursor == openPage->indexCapacity ||
            openPage->recordCursor == openPage->recordCapacity)
        {
            openPage->sealRequested = true;
        }
...
void ChunkManager::Impl::requestSealExactPage(std::uint32_t pageIndex)
{
    std::lock_guard<std::mutex> pageLock(bufferPageMutex_);
    if (pageIndex >= bufferPages_.size())
    {
...
void ChunkManager::Impl::collectSealedExactPages(UINT64 completedGpuFenceValue)
{
    std::vector<std::uint32_t> candidatePageIndices;
    {
        std::lock_guard<std::mutex> pageLock(bufferPageMutex_);
        for (std::uint32_t pageIndex = 0; pageIndex < bufferPages_.size(); ++pageIndex)
        {
            ...
            if (page.state == ChunkBufferPageState::OpenWritable && page.sealRequested)
            {
            ...
            candidatePageIndices.push_back(pageIndex);
        }
    }

    if (candidatePageIndices.empty())
    {
        return;
    }
...
std::size_t ChunkManager::Impl::estimateChunkRetainedBytes(const Chunk& chunk) noexcept
{
    return sizeof(Chunk) +
           ...
           chunk.gpuExactSparseVoxels.capacity() * sizeof(GpuExactSparseVoxel) +
...
std::size_t ChunkManager::Impl::estimatePendingExactGpuBuilds() const
{
    ...
    {
        (void)coord;
        if (!chunk || shouldKeepChunkInteractive(chunk->coord, lastCenterChunk_))
        {
            continue;
        }
                                                              ...
                                                              int jobBudget)
{
    ...
    while (jobBudget > 0)
    {
        ...
        if (requireAccurateStructureReady)
        {
            ...
            {
                // Keep warming the support caches, but do not block exact-only chunk generation on them.
                // Nonlocal exact chunk inputs are authored from CPU descriptors and sparse structure spans,
                // so a missing column-height cache should not strand the last demanded chunks in Missing.
        ...
        const EnsureChunkAsyncResult ensureResult =
            ensureChunkAsync(entry.coord, demandSnapshot.forceResident, localInteraction);
        if (ensureResult == EnsureChunkAsyncResult::EnqueuedGenerate)
        {
                                                     ...
                                                     bool warmStructuresOnly) const
{
    ...
    {
        dependencyPriority = DependencyPriority::Critical;
    }
    else if (priority >= ColumnHeightPrefetchPriority::Visible)
    {
        dependencyPriority = DependencyPriority::Visible;
    }

    const bool warmStructures =
...
std::shared_ptr<const ChunkManager::Impl::WorldgenPage>
ChunkManager::Impl::getOrBuildWorldgenPageBlocking(const glm::ivec2& pageKey,
                                                   WorldgenPageAccessBenchmark* benchmark) const
{
    assert(!gChunkManagerInsideGenerateJob &&
           "Generate jobs must not use getOrBuildWorldgenPageBlocking()");
    if (const std::shared_ptr<const WorldgenPage> readyPage = tryGetReadyWorldgenPage(pageKey))
    {
        return readyPage;
    }
    ...
    const auto buildStart = benchmark != nullptr ? SteadyClock::now() : SteadyClock::time_point{};
    try
    {
        const std::shared_ptr<WorldgenPage> page = buildWorldgenPage(pageKey);
        if (benchmark != nullptr)
        {
            benchmark->buildMicros += static_cast<std::uint64_t>(
                std::chrono::duration_cast<std::chrono::microseconds>(SteadyClock::now() - buildStart).count());
        }
        const_cast<ChunkManager::Impl*>(this)->publishWorldgenPageReady(pageKey, page);
        return page;
    }
    catch (...)
    {
        const_cast<ChunkManager::Impl*>(this)->publishWorldgenPageFailure(pageKey, std::current_exception());
        throw;
    }
}
    ...
    if (!missingWorldgenPageKeys.empty())
    {
        ...
        if (benchmarkEnabled)
        {
            benchmarkMetrics_.chunkDependencyWorldgenDeferrals.record(
                static_cast<std::uint64_t>(missingWorldgenPageKeys.size()));
        }
        ...
        for (int sampleX = -1; sampleX <= kChunkSizeX; ++sampleX)
        {
            for (int sampleZ = -1; sampleZ <= kChunkSizeZ; ++sampleZ)
            {
                ...
                const std::shared_ptr<const WorldgenPage> page =
                    getOrBuildWorldgenPageBlocking(worldgenPageKeyForWorld(sampleWorldX, sampleWorldZ));
                const int localPageX = sampleWorldX - page->baseWorld.x;
                const int localPageZ = sampleWorldZ - page->baseWorld.y;
                worldgenColumns[sampleIndex(sampleX + 1, sampleZ + 1)] = page->column(localPageX, localPageZ);
            }
                                                                                ...
                                                                                bool localInteractionPriority)
{
    ...
    try
    {
        constexpr bool kInitialReadyPriority = true;
        const bool benchmarkEnabled = benchmarkMetrics_.isEnabled();
        ...
        return EnsureChunkAsyncResult::EnqueuedGenerate;
    }
    catch (const std::exception& ex)
    {
        ...
        if (!stalePending && pendingMesh.pageIndex < bufferPages_.size())
        {
            ...
            if (pendingMesh.pageIndex < bufferPages_.size())
            {
                ...
                if (!stalePending)
                {
                    ...
                    page.pendingBatchId = 0;
                    releaseChunkBufferPageUploadBuffers(page);
                    noteExactGpuPageMetadataChange(pendingMesh.pageIndex);
                }
            }
        }
        if (benchmarkEnabled)
    ...
    std::span<const BlockEditOverlayEntry> blockOverlays)
{
    ...
    gpuSparseVoxels.reserve(chunk.structureVoxelEdits.size() +
                            ...
                            blockOverlays.size());
    auto appendSparseVoxel = [&](const glm::ivec3& local,
                                 BlockId block,
                                 bool replaceSolid)
    {
        if (local.x < 0 || local.x >= kChunkSizeX ||
    ...
    for (const StructureVoxelEdit& edit : chunk.structureVoxelEdits)
    {
        appendSparseVoxel(localBlockCoords(edit.worldPos, chunk.coord), edit.block, edit.replaceSolid);
    }
    for (const PendingStructureEdit& edit : pendingStructureEdits)
    {
        appendSparseVoxel(localBlockCoords(edit.worldPos, chunk.coord), edit.block, edit.replaceSolid);
    }
    for (const BlockEditOverlayEntry& overlay : blockOverlays)
    {
        ...
        appendSparseVoxel(glm::ivec3(localX, localY, localZ), overlay.block, true);
    }
...
bool ChunkManager::Impl::refreshChunkGpuExactInputs(Chunk& chunk)
{
...
void ChunkManager::Impl::submitPendingExactGpuBuilds()
{
    ...
    std::uint64_t worldgenResolveMicros = 0u;
    int worldgenPageMisses = 0;
    ...
    const auto recordSubmitStats = [&](bool didWork)
    {
        ...
        if (worldgenResolveMicros > 0u)
        {
            benchmarkMetrics_.exactGpuWorldgenResolveStage.recordMicros(worldgenResolveMicros);
        }
        benchmarkMetrics_.exactGpuSubmitCpuStage.recordMicros(orchestrationMicros);
        if (worldgenPageMisses > 0)
        {
            benchmarkMetrics_.exactGpuWorldgenPageMisses.record(static_cast<std::uint64_t>(worldgenPageMisses));
        }
        if (submitBatchBuilds > 0)
        {
            benchmarkMetrics_.exactGpuSubmitBatchBuilds.record(static_cast<std::uint64_t>(submitBatchBuilds));
        }
        std::lock_guard<std::mutex> statsLock(exactGpuStatsMutex_);
        if (prepareMicros > 0u)
        {
        ...
        exactGpuWorldgenResolveMsLastCycle_ = static_cast<double>(worldgenResolveMicros) / 1000.0;
        exactGpuWorldgenPageMissesLastCycle_ = worldgenPageMisses;
        exactGpuSubmitBatchBuildsLastCycle_ = submitBatchBuilds;
    };
    ...
    auto requeuePreparedBuildRequest = [&](const PreparedExactGpuBuild& prepared)
    {
        ...
        pendingExactGpuBuildQueue_.push_front(
            ExactGpuBuildRequest{prepared.chunk,
    ...
    auto tryStageDescriptorInputs = [&](std::size_t buildIndex) -> bool
    {
        ...
        for (std::size_t pageSlot = 0; pageSlot < pageWindow.pageKeys.size(); ++pageSlot)
        {
            ...
            {
                const SteadyClock::time_point worldgenResolveStart =
                    benchmarkEnabled ? SteadyClock::now() : SteadyClock::time_point{};
                resolvedPages[pageSlot] = getOrBuildWorldgenPageBlocking(pageKey);
                if (benchmarkEnabled)
                {
                    worldgenResolveMicros += static_cast<std::uint64_t>(
                        std::chrono::duration_cast<std::chrono::microseconds>(SteadyClock::now() - worldgenResolveStart).count());
                }
                ++worldgenPageMisses;
            }
            if (!resolvedPages[pageSlot])
        ...
        const std::uint32_t sparseVoxelCount =
            static_cast<std::uint32_t>(chunk.gpuExactSparseVoxels.size());
        if (pending.rebuildVoxelInputs && sparseVoxelCount > 0u)
        {
            const std::uint64_t sparseBytes =
    ...
    for (std::size_t buildIndex = 0; buildIndex < stagedBuilds.size(); ++buildIndex)
    {
        ...
        const std::uint32_t sparseVoxelCount = pending.rebuildVoxelInputs
            ? static_cast<std::uint32_t>(chunk.gpuExactSparseVoxels.size())
            : 0u;
        ...
        prepassRecords.push_back(GpuExactPrepassRecord{
            ...
            pending.rebuildVoxelInputs ? 1u : 0u,
            sparseVoxelCount,
            resolvedNeighborMask,
            kClosedNeighborMask,
            pending.pendingNeighborMask,
            ...
            descriptorIndices.neighborVoxelSrvDescriptorIndices[0],
            descriptorIndices.neighborVoxelSrvDescriptorIndices[1],
            descriptorIndices.neighborVoxelSrvDescriptorIndices[2],
            descriptorIndices.neighborVoxelSrvDescriptorIndices[3],
            descriptorIndices.neighborVoxelSrvDescriptorIndices[4],
            descriptorIndices.neighborVoxelSrvDescriptorIndices[5],
            0u});
    }
    ...
    {
        Chunk& chunk = *pending.chunk;
        if (pending.rebuildVoxelInputs && chunk.exactGpu.voxelState != D3D12_RESOURCE_STATE_UNORDERED_ACCESS)
        {
...
void ChunkManager::Impl::commitPendingExactGpuBuilds()
{
    ...
    std::uint64_t emitMetadataSyncMicros = 0u;
    std::uint64_t prepassFaceTotalsReadbackMicros = 0u;
    std::uint64_t pageSweepMicros = 0u;
    std::uint64_t emitWaitMicros = 0u;
    std::uint64_t emitFenceLifetimeMicros = 0u;
    int emitMetadataDirtyPages = 0;
    ...
    std::size_t emitMetadataUploadBytes = 0u;
    ...
    collectSealedExactPages(completedGpuFenceValue);
    ...
    if (!pendingExactGpuBatches_.empty())
    {
                                                             ...
                                                             const FarLodGpuContext::ExactCompletionEntry& completion) -> bool
        {
            return completion.pageIndex == pending.allocation.pageIndex &&
                   ...
                   static_cast<std::size_t>(completion.vertexBase) == pending.allocation.vertexOffset &&
                   static_cast<std::size_t>(completion.indexBase) == pending.allocation.indexOffset &&
        ...
        for (std::size_t i = 0; i < pendingExactGpuBatches_.size(); ++i)
        {
            ...
            if (batch.phase == ExactGpuBatchPhase::PrepassSubmitted)
            {
                ...
                if (benchmarkEnabled)
                {
                    prepassFaceTotalsReadbackMicros += static_cast<std::uint64_t>(
                        std::chrono::duration_cast<std::chrono::microseconds>(
                            SteadyClock::now() - prepassReadbackStart)
                            .count());
                }
                ...
                if (benchmarkEnabled)
                {
                    batch.prepassReadyForEmitAt = SteadyClock::now();
                }
            }
            ...
            if (keepBatch && batch.phase == ExactGpuBatchPhase::PrepassReadyForEmit)
            {
                ...
                if (benchmarkEnabled && batch.prepassReadyForEmitAt != SteadyClock::time_point{})
                {
                ...
                if (exactGpuContext_.begin())
                {
                    ...
                    else
                    {
                        ...
                        if (!batch.builds.empty())
                        {
                            ...
                            if (benchmarkEnabled)
                            {
                                emitMetadataSyncMicros += static_cast<std::uint64_t>(
                                    std::chrono::duration_cast<std::chrono::microseconds>(SteadyClock::now() - emitMetadataSyncStart).count());
                                emitMetadataDirtyPages += static_cast<int>(std::min<std::size_t>(
                                    emitMetadataSyncDebug.dirtyPageCount,
                                    static_cast<std::size_t>(std::numeric_limits<int>::max() - std::max(emitMetadataDirtyPages, 0))));
                                emitMetadataUploadBytes += emitMetadataSyncDebug.uploadBytes;
                            }
                            if (keepBatch)
                            {
            ...
            {
                if (benchmarkEnabled && batch.emitSubmittedAt != SteadyClock::time_point{})
                {
                    emitFenceLifetimeMicros += static_cast<std::uint64_t>(
                        std::chrono::duration_cast<std::chrono::microseconds>(SteadyClock::now() - batch.emitSubmittedAt).count());
                }
        ...
        if (benchmarkEnabled && hadPendingBuilds)
        {
            ...
            if (emitMetadataSyncMicros > 0u)
            {
                benchmarkMetrics_.exactGpuEmitMetadataSyncStage.recordMicros(emitMetadataSyncMicros);
            }
            if (prepassFaceTotalsReadbackMicros > 0u)
            {
                benchmarkMetrics_.exactGpuPrepassFaceTotalsReadbackStage.recordMicros(prepassFaceTotalsReadbackMicros);
            }
            if (pageSweepMicros > 0u)
            {
                benchmarkMetrics_.exactGpuPageSweepStage.recordMicros(pageSweepMicros);
            }
            if (emitWaitMicros > 0u)
            {
                benchmarkMetrics_.exactGpuEmitWaitStage.recordMicros(emitWaitMicros);
            }
            if (emitFenceLifetimeMicros > 0u)
            {
                benchmarkMetrics_.exactGpuEmitFenceLifetimeStage.recordMicros(emitFenceLifetimeMicros);
            }
            if (emitMetadataDirtyPages > 0)
            {
                benchmarkMetrics_.exactGpuEmitMetadataDirtyPages.record(
                    static_cast<std::uint64_t>(emitMetadataDirtyPages));
            }
            if (emitMetadataUploadBytes > 0u)
            {
                benchmarkMetrics_.exactGpuEmitMetadataUploadBytes.record(
                    static_cast<std::uint64_t>(emitMetadataUploadBytes));
            }
            if (pageSweepPages > 0)
            {
                benchmarkMetrics_.exactGpuPageSweepPages.record(static_cast<std::uint64_t>(pageSweepPages));
            }
            if (emitBatchBuilds > 0)
            {
                benchmarkMetrics_.exactGpuEmitBatchBuilds.record(static_cast<std::uint64_t>(emitBatchBuilds));
            }
            std::lock_guard<std::mutex> statsLock(exactGpuStatsMutex_);
            exactGpuCommitCpuMsLastCycle_ = static_cast<double>(commitMicros) / 1000.0;
            exactGpuEmitMetadataSyncMsLastCycle_ = static_cast<double>(emitMetadataSyncMicros) / 1000.0;
            exactGpuPrepassFaceTotalsReadbackMsLastCycle_ =
                static_cast<double>(prepassFaceTotalsReadbackMicros) / 1000.0;
            exactGpuPageSweepMsLastCycle_ = static_cast<double>(pageSweepMicros) / 1000.0;
            exactGpuEmitWaitMsLastCycle_ = static_cast<double>(emitWaitMicros) / 1000.0;
            exactGpuEmitFenceLifetimeMsLastCycle_ = static_cast<double>(emitFenceLifetimeMicros) / 1000.0;
            exactGpuEmitMetadataDirtyPagesLastCycle_ = emitMetadataDirtyPages;
            ...
            exactGpuEmitMetadataUploadBytesLastCycle_ = emitMetadataUploadBytes;
        }
        return;
    }
}
...
void ChunkManager::Impl::buildChunkMeshAsync(Chunk& chunk)
{
    ...
    for (int localX = 0; localX < kChunkSizeX; ++localX)
    {
        for (int localZ = 0; localZ < kChunkSizeZ; ++localZ)
        {
            const ColumnSample columnSample = sampleColumnBlocking(baseWorldX + localX,
                                                           baseWorldZ + localZ,
                                                           baseWorldY,
...
void ChunkManager::Impl::relightChunkRegion(const PendingRelightBatch& batch)
{
    ...
    regionChunks.erase(std::remove_if(regionChunks.begin(),
                                      regionChunks.end(),
                                      [](const std::shared_ptr<Chunk>& chunk)
                                      {
                                          return !chunk ||
                                                 chunk->residencyMode.load(std::memory_order_acquire) !=
...
bool ChunkManager::Impl::trySampleColumnReady(ColumnSample& outSample,
                                              int worldX,
                                              int worldZ,
                                              int slabMinWorldY,
                                              int slabMaxWorldY,
                                              bool includeBlendDebug) const
{
    ...
    const std::shared_ptr<const WorldgenPage> page =
        tryGetReadyWorldgenPage(worldgenPageKeyForWorld(worldX, worldZ));
    if (!page)
    {
        return false;
    }
    ...
    const int localX = worldX - page->baseWorld.x;
    const int localZ = worldZ - page->baseWorld.y;
    const WorldgenColumnValue worldgenColumn = page->column(localX, localZ);
...
ColumnSample ChunkManager::Impl::sampleColumnBlocking(
    int worldX,
    int worldZ,
    int slabMinWorldY,
    int slabMaxWorldY,
...
{
    assert(!gChunkManagerInsideGenerateJob &&
           "Generate jobs must not use sampleColumnBlocking()");
    const bool benchmarkEnabled = benchmarkMetrics_.isEnabled();
    const SteadyClock::time_point sampleStart = benchmarkEnabled ? SteadyClock::now() : SteadyClock::time_point{};
    ColumnSample sample{};
    if (!trySampleColumnReady(sample, worldX, worldZ, slabMinWorldY, slabMaxWorldY, includeBlendDebug))
    {
        const std::shared_ptr<const WorldgenPage> page =
            getOrBuildWorldgenPageBlocking(worldgenPageKeyForWorld(worldX, worldZ));
        const int localX = worldX - page->baseWorld.x;
        const int localZ = worldZ - page->baseWorld.y;
        const WorldgenColumnValue worldgenColumn = page->column(localX, localZ);
        const bool usesDefaultSlabBounds =
...
void ChunkManager::Impl::registerDeferredChunkDependencies(
    const glm::ivec3& coord,
    std::uint32_t generationEpoch,
    bool initialReadyPriority,
    bool localInteractionPriority,
    ...
    std::span<const StructureRegionKey> missingStructureRegions)
{
    auto clearLocked = [&](auto it)
    {
        if (it == deferredChunkDependencies_.end())
        {
            return;
        }

        for (const glm::ivec2& pageKey : it->second.missingWorldgenPages)
        {
            auto waiterIt = worldgenPageWaiters_.find(pageKey);
            if (waiterIt == worldgenPageWaiters_.end())
            {
    ...
    std::lock_guard<std::mutex> lock(deferredChunkDependencyMutex_);
    clearLocked(deferredChunkDependencies_.find(coord));
    if (missingWorldgenPages.empty() && missingStructureRegions.empty())
    {
        return;
    }

    DeferredChunkDependencies& deferred = deferredChunkDependencies_[coord];
    ...
    {
        if (deferred.missingWorldgenPages.insert(pageKey).second)
        {
            worldgenPageWaiters_[pageKey].insert(coord);
            ++deferred.outstandingDependencyCount;
        }
    }
...
void ChunkManager::Impl::clearDeferredChunkDependencies(const glm::ivec3& coord)
{
    std::lock_guard<std::mutex> lock(deferredChunkDependencyMutex_);
    auto it = deferredChunkDependencies_.find(coord);
    if (it == deferredChunkDependencies_.end())
    {
        return;
    }
...
void ChunkManager::Impl::recordBlockEditOverlay(const glm::ivec3& worldPos, BlockId block)
{
    ...
    if (auto chunk = getChunkShared(chunkCoord))
    {
        chunk->exactGpuInputsVersion.fetch_add(1, std::memory_order_acq_rel);
        chunk->exactGpuInputsDirty.store(true, std::memory_order_release);
    }
...
void ChunkManager::Impl::updateDenseChunkResidency(const glm::ivec3& centerChunk)
{
    ...
    for (const std::shared_ptr<Chunk>& chunk : chunks)
    {
        ...
        const bool keepInteractive = shouldKeepChunkInteractive(chunk->coord, centerChunk);
        ...
        const bool keepDense = keepInteractive &&
...
ID3D12Fence* ChunkManager::exactGpuFence() const noexcept
{
    return impl_->exactGpuFence();
}

std::uint64_t ChunkManager::lastSubmittedExactGpuFenceValue() const noexcept
{
    return impl_->lastSubmittedExactGpuFenceValue();
}
