...
FarLodColumnSample evaluateFarLodColumn(const FarLodGpuWorldgenHeader& header,
                                        std::span<const FarLodGpuBiome> biomes,
                                        int worldX,
                                        int worldZ)
{
    FarLodColumnSample result{};
    if (biomes.empty())
    {
        result.surfaceBlock = BlockId::Air;
        result.fillerBlock = BlockId::Air;
        return result;
    }

    const auto warped = warpPosition(header, worldX, worldZ);
    const float warpedX = warped[0];
    const float warpedZ = warped[1];

    const float continental = fbm2(warpedX, warpedZ, header.mainNoise, header.seed + 23u, kContinentalScale);
    const float temperature01 = std::clamp(fbm2(warpedX + 101.7f, warpedZ - 73.1f, header.mediumNoise, header.seed + 41u, kTemperatureScale) * 0.5f + 0.5f, 0.0f, 1.0f);
    const float moisture01 = std::clamp(fbm2(warpedX - 211.4f, warpedZ + 39.5f, header.mediumNoise, header.seed + 59u, kMoistureScale) * 0.5f + 0.5f, 0.0f, 1.0f);
    const float fertility01 = std::clamp(fbm2(warpedX + 19.5f, warpedZ + 311.9f, header.detailNoise, header.seed + 71u, kFertilityScale) * 0.5f + 0.5f, 0.0f, 1.0f);
    const float mountain01 = std::clamp(ridge2(warpedX, warpedZ, header.mountainNoise, header.seed + 83u), 0.0f, 1.0f);
    ...
    const float inland01 = std::clamp(std::abs(signedDistanceToCoast) / 96.0f, 0.0f, 1.0f);

    std::uint32_t bestBiomeIndex = 0u;
    float bestScore = -1.0f;
    for (std::uint32_t biomeIndex = 0; biomeIndex < biomes.size(); ++biomeIndex)
    {
        const float score = scoreBiome(biomes[biomeIndex],
                                       oceaniness,
                                       temperature01,
                                       moisture01,
                                       fertility01,
    ...
    const FarLodGpuBiome& biome = biomes[bestBiomeIndex];
    const bool isOcean = (biome.flags & kFarLodBiomeOcean) != 0u;
    const CoastProfileSettings& coastSettings = coastProfileSettings(static_cast<FarLodCoastProfile>(biome.coastProfile));

    const float continental01 = std::clamp(continental * 0.5f + 0.5f, 0.0f, 1.0f);
    float landBaseHeight =
        std::lerp(biome.minHeight, biome.maxHeight, continental01) + biome.heightOffset + continental * biome.heightScale;
    float oceanBaseHeight = static_cast<float>(header.seaLevel) - 28.0f + oceaniness * 12.0f;
    float baseHeight = solveShorelineBaseHeight(signedDistanceToCoast,
                                                landBaseHeight,
                                                oceanBaseHeight,
                                                static_cast<float>(header.seaLevel),
                                                coastSettings);

    float roughStrength = std::max(biome.roughness, 0.0f);
    float hillStrength = std::max(biome.hills, 0.0f);
    float mountainStrength = std::max(biome.mountains, 0.0f);
    const float absCoastDistance = std::isfinite(signedDistanceToCoast)
        ? std::abs(signedDistanceToCoast)
    ...
    const float roughNoise = fbm2(warpedX, warpedZ, header.detailNoise, header.seed + 97u, 1.0f);
    const float hillNoise = fbm2(warpedX, warpedZ, header.mediumNoise, header.seed + 113u, 1.0f);
    const float mountainNoise = ridge2(warpedX, warpedZ, header.mountainNoise, header.seed + 131u);
    ...
    const int surfaceY = static_cast<int>(std::round(surfaceHeight));

    BlockId surfaceBlock{};
    BlockId fillerBlock{};
    const float distanceToShore = isOcean ? std::abs(signedDistanceToCoast) : std::max(signedDistanceToCoast, 0.0f);
    ...
    result.biomeIndex = bestBiomeIndex;
    result.hasSolid = true;
    result.waterEnabled = (biome.flags & kFarLodBiomeWaterFill) != 0u;
    result.dominantIsOcean = isOcean;
    result.surfaceY = surfaceY;
    result.waterBottomY = std::max(surfaceY + 1, header.seaLevel - biome.waterMaxDepth + 1);
    result.distanceToShore = distanceToShore;
    result.surfaceBlock = surfaceBlock;
    result.fillerBlock = fillerBlock;
    return result;
}
